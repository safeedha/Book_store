...
const CreateOrder = async (req, res, next) => {
  try {
    const { price, address_id, payment_methods, coupenId, actalprice } =
      req.body;
    const { id } = req.user;
      ...
      if (existingUser) {
        if (existingUser.count >= coupen.usageLimit) {
          return res
            ...
            .json({
              message:
                'You have already applied this coupon, no more uses allowed.',
            });
        } else {
    ...
    const orderItems = cart.map((item) => {
      let discount = item.product_id.offerId
    ...
    const order = await Order.create({
      ...
      payment_status:
        payment_methods === 'cash on delivery' ? 'unpaid' : 'paid',
      total_amount: price,
  ...
  try {
    ...
    if (
      (order.payment_methods === 'online payment' ||
        order.payment_methods === 'wallet payment') &&
      order.coupen_id
    ) {
      let updatedOrderItems = order.order_item.filter(
        (item) =>
      ...
      if (sumofremaining >= order.coupen_id.minimumPurchase) {
        ...
        if (order.coupen_id.coupenType === 'flat') {
          ...
          if (
            amount &&
            order.coupen_id &&
            amount < order.coupen_id.discountedAmount
          ) {
            let value = Math.abs(amount - order.coupen_id.discountedAmount);
            order.remaining = value;
          }
