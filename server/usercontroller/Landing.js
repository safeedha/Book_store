...
const newArrival = async (req, res, next) => {
  try {
    ...
    res.status(200).json({ message: "New products fetched", product: filteredProducts });
  } catch (error) {
    next(error);
  }
};
...
const getAllProduct = async (req, res, next) => {
  try {
    const { priceSort, alphabetSort, categoryList, search } = req.query;
    ...
    let filteredProducts = products.filter(product => product.categoryId !== null);
    ...
    res.status(200).json({ message: 'Products fetched successfully', product: filteredProducts });
  } catch (error) {
    next(error);
  }
};
...
const getsingleProduct=async(req,res,next)=>{
  try{
   const { id } = req.params; 
   const product=await Product.findOne({_id:id}).populate('categoryId', 'name').populate("offerId")  
