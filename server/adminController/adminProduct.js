...
const addProduct = async (req, res,next) => {
  try {
    const { formatProductName, description, author, language, price, category, stock, sku, uploaded } = req.body;
...
const getsingleProduct=async(req,res,next)=>{
  try{
   const { id } = req.params; 
   const product=await Product.findOne({_id:id}).populate('categoryId', 'name') 
...
const updateProduct = async (req, res,next) => {
  try {
    ...
    const exist = await Product.findOne({ name: formatProductName, _id: { $ne: id } });
    if (exist) {
