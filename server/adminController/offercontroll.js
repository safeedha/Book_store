...
const createcategoryOffer = async (req, res, next) => {
  try {
    ...
    await category.save();
    const product = await Product.find({ categoryId: catid }).populate(
      'offerId'
    );
    console.log(product);
    for (let pro of product) {
      if (pro.offerId === null) {
        ...
        let updatedOfferId =
          pro.offerId.offerAmount >= offer.offerAmount
            ? pro.offerId._id
            : offer._id;
...
const createproductOffer = async (req, res, next) => {
  try {
    ...
    if (category.offerId?._id) {
      ...
      console.log();
      let updatedOfferId =
        category.offerId.offerAmount >= offer.offerAmount
          ? category.offerId._id
          : offer._id;
