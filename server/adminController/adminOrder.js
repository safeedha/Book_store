...
const getOrder=async(req,res,next)=>{
  try{
     const order=await Order.find({}).populate('user_id').populate('shipping_address').populate({path:'order_item.product_id',select: 'name images'}).sort({
      updatedAt:-1})
     res.status(200).json({sucsess:true,order})
   }
 catch(error)
 {
   next(error)
...
const OrderStatus = async (req, res, next) => {
  try {
    ...
    const orderItem = order.order_item.find(item => item.product_id.toString() === prodId);
            ...
            if(order.total_amount<order.actual_amount)
            {
              ...
              if(sumofremaining>=order.coupen_id.minimumPurchase)
              {
                ...
                if(order.coupen_id.coupenType==='discount')
                  {
                    order.total_amount=Math.ceil(sumofremaining-Math.ceil(sumofremaining*order.coupen_id.discountedAmount/100))
                  }
                  amount=Math.ceil((orderItem.original_price-Math.ceil(orderItem.original_price*orderItem.discount/100))*orderItem.quantity)
...
const singleOrderdetail=async(req,res,next)=>{
   try{
     ...
     const response={
       ...
       TotalPrice:(Math.ceil(productDetails.original_price-(productDetails.original_price*productDetails.discount/100)))*productDetails.quantity,
