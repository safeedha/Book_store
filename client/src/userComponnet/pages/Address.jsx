...
import { getAddress,addAddress,deletAddress} from '@/User_apiservices/Address';
  ...
  const item = address.map((add, index) => (
    <div className='bg-white w-48 p-4 rounded-lg shadow-2xl' key={add._id}>
      <div className='space-y-2'>
        ...
        <p className='text-sm text-gray-700 overflow-hidden'>{add.streetAddress}</p>
        <p className='text-sm text-gray-700'>{add.city}</p>
        <p className='text-sm text-gray-700'>{add.state} {add.district} {add.pincode}</p>
        <p className='text-sm text-gray-700'>Phone number: {add.phone}</p>
      </div>
      <div className='mt-4 flex justify-between text-sm text-blue-600'>
        <span className='cursor-pointer hover:text-blue-800' onClick={()=>EditHandle(add._id)}>Edit</span>
        <span className='cursor-pointer hover:text-red-600' onClick={()=>RemoveHandle(add._id)}>Remove</span>
      </div>
    </div>
  ));
    ...
    <div className='flex'>
      ...
      >
        <div className='border-2 border-indigo-600 h-16 pt-4 pl-4'>
          <button className='text-lg font-semibold text-sky-900' onClick={addressHandle}>
            +ADD A NEW ADDRESS
          </button>
        </div>

        {add && (
         <form onSubmit={handleSubmit} className="space-y-4 border-indigo-600 mt-2">
