...
import { wishListadd, wishListremove,WishListSatus } from '@/User_apiservices/wishlist';
...
function Singleproduct() {
  ...
  useEffect(() => {
    const getSingleProduct = async () => {
      try {
        await productInformation(id, 
          setProductName, setDescription, setAuthor, setLanguage, setPrice, setStock, setOpt, set_Id, setSku, setImage, setActiveImage,toast,setOffer
        );     
      } catch (error) {
        console.error(error);
      }
  ...
  const cartHandle=async()=>{
    ...
    catch(error)
    {
      ...
      if(error.response.data.message==="The requested quantity not availabel")
      {
       return  toast.error(error.response.data.message)
      }
        if(error.response.data.message==="Oops! There's a limit of 5 units per person for this product. Adjust the quantity to proceed.")
        {
         return  toast.error("Oops! There's a limit of 5 units per person for this product. Adjust the quantity to proceed.")
        }
    }
  }
  ...
  <div className="flex-1 w-full lg:w-1/2 items-center mt-10 pt-16 relative">
   <div className="bg-white border border-red-600 shadow-lg rounded-lg h-72 flex justify-center items-center relative">
    ...
    {activeImage && (
      <Lens hovering={hovering} setHovering={setHovering} className="mt-4">
        <img
          src={activeImage || ""}
          alt=""
        ...
        />
          ...
          <div className="bg-white shadow-lg rounded-lg p-4 w-full h-max lg:w-1/3 relative">
            <h1 className="text-xl font-bold text-gray-800 mb-2">{productName} </h1>
            ...
            {offer && (
              <div className="text-sm mb-1 text-green-600">
                ...
                <br></br>
                <span className="text-green-600 font-semibold"> {offer}% offer</span>
              </div>
            )}
            <div className="text-gray-700 text-sm mb-1">
    ...
    <p className="text-gray-600 ml-2">(50 Reviews)</p>
      ...
      <div className="space-y-4">
        ...
        <div className="border-b pb-4">
          ...
          <p className="text-gray-600 mt-2">Great product! Highly recommended.</p>
        </div>
  ...
  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
    {relatedProducts.map((product) => (
      <div key={product._id} className="border rounded-lg p-4 bg-white shadow-md " onClick={()=>singleProduct(product._id)}>
        <img
