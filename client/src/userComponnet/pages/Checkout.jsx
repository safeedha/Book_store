...
function Checkout() {
      ...
      if (result.length === 0) {
        ...
        if (response) {
          ...
          if (result[0].coupenType === 'percentage') {
            newprice =
              price - Math.ceil((price * result[0].discountedAmount) / 100);
  ...
  useEffect(() => {
    const result = cartproduct.reduce((accumulator, current) => {
      const isOfferValid =
        new Date(current.product_id.offerId?.expiryDate) > new Date();
      const price = isOfferValid
        ? Math.ceil(
            current.product_id.price -
              (current.product_id.price *
                current.product_id.offerId.offerAmount) /
                100
          )
        : current.product_id.price;
    ...
    >
      ...
      {item.coupenType === 'flat' && (
        <p className="text-gray-700 mt-2">
          Discounted Amount:{' '}
          <span className="font-bold">₹{item.discountedAmount}</span>
        </p>
      )}
  ...
  const item = address.map((add) => (
    <div className="bg-white w-full md:w-48 p-3 rounded shadow" key={add._id}>
      ...
      <div className="text-sm">
        ...
        <p>
          {add.city}, {add.state} {add.district}
        </p>
    ...
    >
      ...
      <div className="flex-shrink-0">
        {(item.product_id.offerId === null ||
          (item.product_id.offerId &&
            new Date(item.product_id.offerId?.expiryDate) < new Date())) && (
            ...
            <p className="text-red-700 font-medium mb-2">
              ...
              {Math.ceil(
                item.product_id.price -
                  (item.product_id.offerId.offerAmount *
                    item.product_id.price) /
                    100
              ) * item.quantity}
            </p>
        ...
        {mod && (
          <div className="border-2 bg-white h-auto w-96 rounded-lg shadow-md p-6 text-center absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
            ...
            <div className="flex justify-center space-x-4">
              ...
              <button
                className="bg-red-300 text-gray-700 px-4 py-2 rounded-lg hover:bg-red-400 transition duration-300"
                onClick={noHandle}
              >
                NO
              </button>
              ...
              <>
                ...
                {add && (
                  <form
                    ...
                    className="space-y-4 border-double border-4 border-indigo-600 mt-2 p-4"
                  >
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-start gap-3 p-3 bg-gray-50 rounded-md hover:shadow-sm border border-gray-200">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500">
                        Use your wallet balance to pay.
                      </p>
                    </div>
                  </div>
            ...
            {coupen.length > 0 && (
              <div>
                ...
                <div className="flex gap-2 w-full">
                  ...
                  />
                  <button
                    className="px-4 py-2 text-white bg-blue-500 rounded-lg hover:bg-blue-600 focus:ring-2 focus:ring-blue-500"
                    onClick={coupenApply}
                  >
                    Apply
                  </button>
                  {coupenId && (
                    <button
                      className="px-4 py-2 text-white bg-red-500 rounded-lg hover:bg-red-600 focus:ring-2 focus:ring-blue-500"
