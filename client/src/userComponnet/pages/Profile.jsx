...
function Profile() {
  ...
  const handleOnsubmit = async (e) => {
    try {
     ...
     await profileUpdate(updatedData,dispatch,setUserDetails,setIsEditing,toast)
    } catch (error) {
      console.log (error)
      ...
      >
        <div className='max-w-md mx-auto'>
          ...
          <form className='space-y-6' onSubmit={handleOnsubmit}>
            <div>
              <label className='block text-red-950 font-semibold'>Name:</label>
              <input
                type="text"
                value={name}
            ...
            <div>
              <label className='block text-red-950 font-semibold'>Mobile No:</label>
              <input
                type="number"
                value={phonenumber}
