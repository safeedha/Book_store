...
import { getSingleOrderdetail,getrelatedOrderdetail } from '../../User_apiservices/orderApi';
  ...
  const realtedorder=related.map((item,index)=>(
    <div className='bg-white shadow-md p-5 rounded-lg' key={index}>
      ...
      <div className="text-xs leading-tight">
        <p className="text-sm font-semibold text-gray-800 truncate">{item.product_id.name}</p>
        <p className="text-gray-600 mt-1">Qty: {item.quantity}</p>
        <p className="mt-1">
          <span className={`text-red-500 text-sm `}>
          &#8377;
          {Math.ceil(item.original_price - (item.original_price * item.discount) / 100)}
          </span>
        </p>
        <p className="mt-1">
          <span className={`text-red-500 text-sm `}>
            Total:
          &#8377;
          {Math.ceil(item.original_price - (item.original_price * item.discount) / 100) * item.quantity}
          </span>
        </p>
  ...
  <div className="flex justify-between items-start mb-2">
    <div className="flex items-center space-x-2">
      ...
      <div className="text-xs leading-tight">
        ...
        <p className="text-gray-600 mt-1">Qty: {orderDetail.quantity}</p>
        <p className="mt-1">
          <span className={`text-red-500 text-sm `}>
          &#8377;{Math.ceil(orderDetail.originalprice - (orderDetail.originalprice * orderDetail.discount) / 100)}
          </span>
        </p>
     
          <p className="text-red-500 text-sm">
           Total:
            &#8377;{(Math.ceil(orderDetail.originalprice - (orderDetail.originalprice * orderDetail.discount) / 100))*orderDetail.quantity}
          </p>
    ...
    <div className="text-xs leading-tight flex-1 ml-4">
      <h3 className="text-sm font-semibold text-gray-800 mb-1">Shipping Info</h3>
      <p>{orderDetail.shippingname}</p>
      <p>{orderDetail.shippingnumber}</p>
      <p>{orderDetail.shippingstreetAddress}</p>
      <p>
        {orderDetail.shippingcity}, {orderDetail.shippingdistrict}, {orderDetail.shippingstate} - {orderDetail.shippingpincode}
      </p>
    </div>
  </div>
...
{related.length > 0 && (
  <>
    <h1 className="text-center font-bold text-2xl mb-4">Other products in this order</h1>
