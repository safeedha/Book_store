...
export default function Dashbord() {
  ...
  useEffect(() => {
    const orderAmount = order.reduce(
      (accum, curr) => accum + (curr.actual_amount + 50),
      0
    );
    setOrderamount(orderAmount);
    const orderNet = order.reduce(
      (accum, curr) => accum + (curr.total_amount + 50),
      0
    );
    ...
    const discount = order.reduce(
      (accum, curr) => accum + (curr.actual_amount - curr.total_amount),
      0
    );
