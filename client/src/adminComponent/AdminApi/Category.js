...
export const updateCategory = async (
  ...
  try {
    const res = await adminInstance.post(`/category/${id}`, {
      formattedCategoryName,
      uploadedUrl,
    });
    if (res.status === 200) {
...
export const AddofferfromCategory = async (
  catid,
  name,
  expiryDate,
  discount,
  ...
  try {
    const response = await adminInstance.post('/offer/category', {
      catid,
      name,
      expiryDate,
      discount,
    ...
    if (error.response.status === 409) {
      toast.error(
        'offer already exist for this category,You can only edit and delete this offer'
      );
      setIsOpen(false);
    }
  }
