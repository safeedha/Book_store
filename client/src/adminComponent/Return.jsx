...
import { returnedProduct,approveProduct,rejectProduct } from './AdminApi/returnHandle'
...
function Return() {
  ...
  const returnedItems=returned.map((item ,index)=>(
    <tr key={index}>
      <td className="border border-gray-300 px-4 py-2" >
        ...
        <p>Price for single:{item.filtered_items.original_price-item.filtered_items.discount}</p>
      </td>
      <td className="border border-gray-300 px-4 py-2">
       {item.user_info.name}
      </td>
      ...
      <th className="border border-gray-300 px-4 py-2">
          ...
          disabled={active}
          onClick={()=>approvedHandle(item._id,item.filtered_items.product_id,item.user_id)}
        >
          APPROVED
        </button>
    ...
    <button
      ...
      disabled={active}
      onClick={()=>rejectHandle(item._id,item.filtered_items.product_id,item.user_id)}
    >
      REJECTED
    </button>
  ...
  return (
    ...
    {returned.length === 0 && (
      ...
      <thead>
        <tr>
          <th className="border border-gray-300 px-4 py-2">Product Information</th>
          <th className="border border-gray-300 px-4 py-2">Customer</th>
          <th className="border border-gray-300 px-4 py-2">Return Reason</th>
          <th className="border border-gray-300 px-4 py-2">
            <div className="flex justify-center space-x-2">
              Action
            </div>
