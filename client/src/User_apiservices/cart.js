...
export const addCartQuantity = async (
  ...
  try {
    const response = await instance.post(`/user/cart/add/${id1}`, {
      productid,
    });
    ...
    if (error.response) {
      ...
      if (message === 'User blocked') {
        toast.error(
          "Your account is blocked. You can't add products to the cart."
        );
        setTimeout(() => {
          dispatch(logoutUser());
        }, 200);
...
export const subCartQuantity = async (
  id1,
  productid,
  setCartproduct,
  dispatch,
  ...
  try {
    const response = await instance.post(`/user/cart/sub/${id1}`, {
      productid,
    });
    if (response.status === 200) {
      setCartproduct(response.data.cart);
    ...
    if (
      error.response.data.message ===
        'This product is out of stock and removed from your cart' ||
      error.response.data.message ===
        'Quantity exceeded stock. Updated to available stock.'
    ) {
      toast.error(error.response.data.message);
